...
that admits no judgement:
    ...
    Splitting one member into two members OF THE SAME FAMILY that carry the same
    value and share its weight describes the same ensemble, and must produce
    identical statistics.

"Of the same family" is load-bearing and was added when Phase 2.1 landed. A
genuinely new forecasting centre SHOULD move the statistics — that is
information arriving. What must not move them is expressing one centre's opinion
as two members, which is exactly what icon_global/icon_eu/icon_d2 do.

Any statistic computed over member COUNT fails this. Any statistic computed over
FAMILY WEIGHT passes it. That is precisely the distinction the gates depend on.
...
# A fixed 4-member ensemble, matching the shipped US blend's weights. Four
# distinct families: ECMWF, NCEP, DWD, CCMEP.
...
# The same ensemble with DWD expressed as two resolutions instead of one. Same
# family, same value, same total weight — so the same ensemble.
SPLIT_W = dict(BASE_W, icon_global=0.10, icon_eu=0.10)
SPLIT_T = dict(BASE_T, icon_eu=BASE_T["icon_global"])
...
class TestSplittingAMemberChangesNothing:
    ...
    def test_weighted_spread_is_unchanged(self):
        assert W._weighted_pstdev(SPLIT_T, SPLIT_W) == pytest.approx(
    ...
    def test_an_unweighted_spread_would_fail_this(self):
        ...
        assert W._pstdev(list(SPLIT_T.values())) != pytest.approx(
            W._pstdev(list(BASE_T.values())), abs=1e-9)

    def test_a_genuinely_NEW_family_does_move_the_statistics(self):
        """The complement, and the reason "of the same family" is in the
        property above. A new forecasting centre is information arriving and
        must be allowed to move the gates; invariance to that would mean the
        ensemble expansion could never change a decision."""
        four = _engine(BASE_T, BASE_W)
        w5 = dict(BASE_W, jma_gsm=0.20)
        t5 = dict(BASE_T, jma_gsm=BASE_T["ecmwf_ifs025"] + 6.0)
        five = _engine(t5, w5)
        assert five["model_spread_std"] != pytest.approx(four["model_spread_std"], abs=1e-6)
